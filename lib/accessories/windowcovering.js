...
module.exports = class WindowCovering {
    ...
    constructor(log, config, accessory, radiora2, homebridge, storage) {
        ...
        this.config = config;
        this.conditionVar = (this.config.keypadHookData?.match(/^(\d+)\?/) || [])[1];
        this.avoid = false;
        this.keypadData = this.config.keypadHookData ? this.config.keypadHookData.replace(/^.*?\?/, "").replace(/[^\d,;]/g,"").split(";").map((x)=>x.split(",").map(parseFloat)) : []
    ...
    setupListeners() {
        ...
        this.radiora2.on("off", function (integrationId) {
            ...
            if (integrationId == this.config.id) {
                this.log.debug(`[${this.config.name}] Closed`);
                this.log.debug(`[${this.config.name}] Closed Aborted`);
                return;
                this.accessory
                    .getService(Service.WindowCovering)
                    .getCharacteristic(Characteristic.TargetPosition)
        ...
        this.radiora2.on("level", function (integrationId, level) {
            ...
            if (integrationId == this.config.id) {
                ...
                this.log.debug(`[${this.config.name}] RadioRA2 Set to ${level}%`);
                this.accessory
                    .getService(Service.WindowCovering)
                    .getCharacteristic(Characteristic.TargetPosition)
                    .updateValue(level);
        ...
        this.radiora2.on("tilt", function (integrationId, level) {
            ...
            if (integrationId == this.config.id) {
                if (this.config.blind) {
                    this.log.debug(`[${this.config.name}] Tilt set to ${level}%`);
                    this.accessory
                        .getService(Service.WindowCovering)
        ...
        this.radiora2.on("moving", function (integrationId, state) {
            ...
            if (integrationId == this.config.id) {
                ...
                if (state == 0) {
                    this.log.debug(`[${this.config.name}] Moving up`);
                    this.accessory
                        .getService(Service.WindowCovering)
                        .getCharacteristic(Characteristic.PositionState)
                        .updateValue(Characteristic.PositionState.INCREASING);
                } else if (state == 1) {
                    this.log.debug(`[${this.config.name}] Moving down`);
                    this.accessory
                        .getService(Service.WindowCovering)
                        .getCharacteristic(Characteristic.PositionState)
                        .updateValue(Characteristic.PositionState.DECREASING);
                } else if (state == 2) {
                    this.log.debug(`[${this.config.name}] Finished moving`);
                    this.accessory
                        .getService(Service.WindowCovering)
                        .getCharacteristic(Characteristic.PositionState)
	...
	// Control covers via keypads
	    ...
	    // Is this for us?
            if (action == 3 && this.keypadData.some(([keypad,lower,raise])=>integrationId==keypad && buttonId == raise)) {
                this.log.info(`[${this.config.name}]   --->   3up`);
                if (this.direction == 2) {
                ...
                this.log.info(`[${this.config.name}]   --->   3down`);
                if (this.direction == 3) {
                    this.stopSimulatedDimmer(true)
                } else {
                    this.setSimulatedDimmer(0,0,true);
                }
            } else if (this.config.blind && action == 4 && this.keypadData.some(([keypad,lower,raise])=>integrationId==keypad && (buttonId == lower || buttonId == raise))) {
		    // handle jog latch
                    this.hold = false;
            }
        }.bind(this));

        if (this.conditionVar != undefined) {
            this.radiora2.on("sysvar", function(sysvar, value) {
                // this.log.info(`[${this.config.name}] DEBUG ${sysvar},${value},${this.conditionVar},${sysvar == this.conditionVar}`);
                if (sysvar == this.conditionVar) {
                    this.avoid = value!=0;
                    this.log.info(`[${this.config.name}] conditional var updated to ${value}`);
                }
            }.bind(this));
            this.radiora2.querySysVarState(this.conditionVar);
        }
    }

    setTargetPosition(level, callback) {
        this.log.debug(`[${this.config.name}] setTargetPosition called with level: ${level}`);
        if (this.avoid) {
            this.log.info(`[${this.config.name}] setTargetPosition aborted due to its conditional var`);
            return;
        }
        setTimeout(function() {
    ...
    stopSimulatedDimmer(control_motors) {
        this.log.info(`[${this.config.name}] Stopping simulated dimmer`);
        this.direction = 0;
        ...
        this.accessory
            ...
            .getCharacteristic(Characteristic.PositionState)
            .updateValue(Characteristic.PositionState.STOPPED);
        this.log.debug(`[${this.config.name}] Stopped simulated dimmer: level ${this.level}, tilt ${this.tilt}`);
    }

    setSimulatedDimmer(target_level, target_tilt, control_motors) {
        if (this.avoid) {
            this.log.info(`[${this.config.name}] setSimulatedDimmer aborted due to its conditional var. Params: target_level=${target_level}, target_tilt=${target_tilt}, control_motors=${control_motors}`);
            return;
        }
        // Already in estimator loop and this is a tilt-only update (e.g. from setTargetTilt): just update target tilt and let current estimator continue
        if (this.estimator != undefined && target_level === undefined && target_tilt !== undefined) {
            this.target_tilt = target_tilt;
            this.log.debug(`[${this.config.name}] setSimulatedDimmer: estimator running, updating target_tilt only to ${target_tilt}`);
            if (this.config.blind) {
                this.accessory
                    .getService(Service.WindowCovering)
                    .getCharacteristic(this.targetTiltAngleCharacteristic)
                    .updateValue(this.target_tilt * 1.8 - 90);
            }
            return;
        }
        this.log.debug(`[${this.config.name}] set Simulated Dimmer: level ${this.level}->${target_level}, tilt: ${this.tilt}->${target_tilt}, control_motors: ${control_motors}`);
        this.hold = true;
	...
	this.direction = direction;
        ...
        //this.log.info(`[${this.config.name}] TARGET: ${this.target_level};${this.target_tilt}`);

        if (this.config.blind && this.target_tilt != undefined) {
            this.accessory
            .getService(Service.WindowCovering)
            .getCharacteristic(this.targetTiltAngleCharacteristic)
            .updateValue(this.target_tilt * 1.8 - 90);
        ...
        .getService(Service.WindowCovering)
        .getCharacteristic(Characteristic.PositionState)
        .updateValue(direction==2 ? Characteristic.PositionState.INCREASING : Characteristic.PositionState.DECREASING);
 
        this.iteration_index = 0;
        this.estimator = setInterval(() => {
	    this.iteration_index++;
            this.log.debug(`[${this.config.name}] ESTIMATOR[${this.iteration_index}]: level ${this.level}, tilt: ${this.tilt}`);
            // Push current position so Siri/Home can read it for "stop" (set target to current)
            if (this.iteration_index % 10 === 0) {
                this.accessory.getService(Service.WindowCovering).getCharacteristic(Characteristic.CurrentPosition).updateValue(this.level);
                this.accessory.getService(Service.WindowCovering).getCharacteristic(Characteristic.TargetPosition).updateValue(this.target_level);
            }
            if (skip-- > 0) return; // skip to simulate delayed motor
            if (this.iteration_index == jog && !this.hold) {
                this.log.info(`[${this.config.name}] JOG STOP`);
            ...
            if (Math.abs(level_diff) > delta) {
                ...
                if (this.level != this.target_level) {
                    ...
                    if (this.config.blind && (this.target_tilt !== undefined) && (this.target_tilt != this.tilt)) {
                        this.log.debug(`[${this.config.name}] Reverting engine's direction to get to the right tilt`);
                ...
                if (this.config.blind) {
                    if (this.target_tilt !== undefined) {
                        ...
                        if (Math.abs(tilt_diff) > delta_tilt) {
                            this.log.info(`[${this.config.name}] tilt ${this.tilt}`);
                            this.tilt += Math.sign(tilt_diff) * delta_tilt;
                            return;
                        } else {
                            this.tilt = this.target_tilt;        
                            this.storage.setItem('tilt-'+this.config.id, this.tilt); 
                            this.log.info(`[${this.config.name}] Tilt Set: ${this.tilt}`);
                            delete this.target_tilt;
                        }
                    }
                }
                this.log.info(`[${this.config.name}] Level Set: ${this.level}`);
                this.stopSimulatedDimmer(control_motors);
            }
       }, delta_ms);
    ...
    setHoldPosition(level, callback) {  
        this.log.debug(`[${this.config.name}] setHoldPosition called with level: ${level}`);
        setTimeout(function() {
            this.log.debug(`[${this.config.name}] setHoldPosition`);
            if (this.config.blind) {
            ...
            setTimeout(function() {this.stopSimulatedDimmer(true);}.bind(this),100)
        }.bind(this),100);
        callback(null);
    }

    setTargetTilt(level,callback) {
        this.log.debug(`[${this.config.name}] setTargetTilt called with tilt level: ${level}`);
       if (this.config.tilt == "vertical") {
    ...
    setTargetHorizontalTilt(level, callback) {
        if (this.avoid) {
            this.log.info(`[${this.config.name}] setTargetHorizontalTilt aborted due to its conditional var`);
            return;
        }
        this.log.debug(`[${this.config.name}] setTargetHorizontalTilt called with level: ${level}`);
    ...
    setTargetVerticalTilt(level, callback) {
        if (this.avoid) {
            this.log.info(`[${this.config.name}] setTargetVerticalTilt aborted due to its conditional var`);
            return;
        }
        this.log.debug(`[${this.config.name}] setTargetVerticalTilt called with level: ${level}`);
