    ...
    setupListeners() {
        ...
        this.radiora2.on("level", function (integrationId, level) {
            ...
            if ((integrationId == this.config.id) && (this.config.adjustable)) {
                ...
                // Hack - if level is exactly 100 (and not 99.99) then it means this was opened throught the lutron system - so we don't want to set the rotation speed
                if (level != 100) {
                    this.accessory
                        .getService(Service.Fan)
                        .getCharacteristic(Characteristic.RotationSpeed)
        ...
        if (powerOn) {
            this.onTimer = setTimeout(function() {
                ...
                if (this.config.adjustable) {
                    ...
                    onValue = this.config.onvalue || this.accessory
                            .getService(Service.Fan)
