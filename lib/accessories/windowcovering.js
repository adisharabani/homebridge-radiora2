...
module.exports = class WindowCovering {
    ...
    setupListeners() {
        ...
        this.radiora2.on("off", function (integrationId) {
            ...
            if (integrationId == this.config.id) {
                ...
                this.log.debug("Closed Aborted", this.config.name);
                return;
                this.accessory
                    .getService(Service.WindowCovering)
                    .getCharacteristic(Characteristic.TargetPosition)
        ...
        this.radiora2.on("level", function (integrationId, level) {
            ...
            if (integrationId == this.config.id) {
                ...
                this.log.debug("RadioRA2 Set to " + level + "%", this.config.name);
                this.accessory
                    .getService(Service.WindowCovering)
                    .getCharacteristic(Characteristic.TargetPosition)
                    .updateValue(level);
	...
	// Control covers via keypads
        ...
        if (this.conditionVar != undefined) {
            this.radiora2.on("sysvar", function(sysvar, value) {
                // this.log.info("DEBUG "+sysvar + ","+value+","+this.conditionVar+","+(sysvar == this.conditionVar))
    ...
    setTargetPosition(level, callback) {
        this.log.debug(`setTargetPosition called with level: ${level}`);
        if (this.avoid) {
            this.log.info("setTargetPosition " + this.config.name + " aborted due to its conditional var")
            return;
        }
        setTimeout(function() {
    ...
    stopSimulatedDimmer(control_motors) {
        ...
        this.accessory
            ...
            .getCharacteristic(Characteristic.PositionState)
            .updateValue(Characteristic.PositionState.STOPPED);
        this.log.debug("Stopped simulated dimmer for " + this.config.name + ": level " + this.level + ", tilt " + this.tilt);
    }

    setSimulatedDimmer(target_level, target_tilt, control_motors) {
        if (this.avoid) {
            this.log.info(`setSimulatedDimmer ${this.config.name} aborted due to its conditional var. Params: target_level=${target_level}, target_tilt=${target_tilt}, control_motors=${control_motors}`);
            return;
        }
        ...
        if (this.estimator != null && target_level === undefined && target_tilt !== undefined) {
            this.target_tilt = target_tilt;
            this.log.debug("setSimulatedDimmer: estimator running, updating target_tilt only to " + target_tilt);
            if (this.config.blind) {
                this.accessory
                    .getService(Service.WindowCovering)
                    .getCharacteristic(this.targetTiltAngleCharacteristic)
        ...
        this.log.debug("set Simulated Dimmer: level " + this.level + "->" + target_level + ", tilt: " + this.tilt + "->" + target_tilt + ", control_motors: " + control_motors);
        this.hold = true;
	...
	this.direction = direction;
        ...
        //this.log.info("TARGET: " + this.target_level + ";" + this.target_tilt);
	    ...
	    this.iteration_index++;
            ...
            if (this.iteration_index == jog && !this.hold) {this.log.info("JOG STOP");this.stopSimulatedDimmer(control_motors); return}
    ...
    setHoldPosition(level, callback) {  
        this.log.debug(`setHoldPosition called with level: ${level}`);
        setTimeout(function() {
    ...
    setTargetTilt(level,callback) {
        this.log.debug(`setTargetTilt called with level: ${level}`);
       if (this.config.tilt == "vertical") {
    ...
    setTargetHorizontalTilt(level, callback) {
        if (this.avoid) {
            this.log.info("setTargetHorizontalTilt " + this.config.name + " aborted due to its conditional var")
            return;
        }
        this.log.debug(`setTargetHorizontalTilt called with level: ${level}`);
    ...
    setTargetVerticalTilt(level, callback) {
        if (this.avoid) {
            this.log.info("setTargetVerticalTilt " + this.config.name + " aborted due to its conditional var")
            return;
        }
        this.log.debug(`setTargetVerticalTilt called with level: ${level}`);
