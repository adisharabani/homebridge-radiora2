...
var RadioRa2 = function (host, username, password, log) {
    ...
    async function incomingData(data) {
        ...
        if ((/^GNET>/.test(str)) || (/^QNET>/.test(str))) {
            ...
            if (components[0] == "~SYSVAR") _processSysvarResponse(components);
    ...
    function _processSysvarResponse(dataComponents) {
        var varId = dataComponents
        log.debug("sysvar: " + dataComponents);
        if (dataComponents.length >= 4) {
            var varId = dataComponents[1]
            var action = dataComponents[2]
            var value = dataComponents[3]
            if (action != 1) return;
            me.emit("sysvar", varId, value);
        }
    }
    ...
    function _processHVACResponse(dataComponents) {
        ...
        if ((dataComponents.length >= 3)) {
            ...
            // state
                ...
                me.emit("sensors", deviceId, parameter);
            }
            // Unknown
            else {
    ...
    function _processOutputResponse(dataComponents) {
        ...
        if (dataComponents.length >= 3 && (dataComponents[2] == 1 || dataComponents[2] == 9 || dataComponents[2] == 10 || dataComponents[2] == 32 || dataComponents[2] == 30)) {
            if (dataComponents[2] == 10) {
                ...
                // XXXX
                } else if (dataComponents[2] == 30) {
                    //log.info("ACTION30 FOUND: " + dataComponents)
                    me.emit("action", integrationId, parseFloat(dataComponents[4]));
    ...
    this.querySysVarState = function (sysVarId) {
        me.sendCommand("?SYSVAR," + sysVarId + ",1");
    };
